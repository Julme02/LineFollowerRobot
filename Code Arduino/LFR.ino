...
int Kp = 25, Ki = 0, Kd = 16;   // Hệ số PID

int base_speed_l = 114;   // Do động cơ bánh trái yếu hơn bánh phải
int base_speed_r = 100;   // nên phải set xung PWM cao hơn thì xe mới đi thẳng
...
void loop(){
  readsensor();
  if(error == 1000){     // quay tròn đến khi tìm lại được đường
    Stop();
    analogWrite(enB, 120);
    TurnLeft();  
  }
  else if(error == 2000){   // Dừng lại khi cả 5 sensor nhận được vạch đen
    Stop();
  }
  else{                  // dò đường
    calculate_pid();
